...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/codepath';
...
async function seedDatabase() {
  // Clear the existing roadmap so it uses our updated seed data
  await Roadmap.deleteMany({});
  console.log("Cleared existing roadmap data.");
  
  console.log("Seeding new roadmap data with enriched details...");
...
app.get('/api/roadmap', async (req, res) => {
  try {
    let data = await Roadmap.findOne();
    if (!data) {
      res.json(seedData);
    } else {
      res.json(data);
